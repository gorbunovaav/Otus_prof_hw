...
class VirtualMachine(object):
    ...
    def byte_CACHE(self, arg):
        pass
    
    def byte_PUSH_NULL(self, arg):
        pass

    def byte_PRECALL(self, arg):
        pass
    ...
    def parse_byte_and_args(self):
        ...
        opoffset = f.f_lasti
        byteCode = byteint(code[opoffset])
        byteName = dis.opname[byteCode]
        f.f_lasti += 1
        arg = None
        if byteCode >= dis.HAVE_ARGUMENT:
            argval = byteint(code[f.f_lasti]) + (byteint(code[f.f_lasti + 1]) << 8)
            f.f_lasti += 2

            if byteCode in dis.hasconst:
                arg = f.f_code.co_consts[argval]
            elif byteCode in dis.hasname:
                arg = f.f_code.co_names[argval] if argval < len(f.f_code.co_names) else f"<InvalidName:{argval}>"
            elif byteCode in dis.haslocal:
                arg = f.f_code.co_varnames[argval]
            elif byteCode in dis.hasfree:
                n_cell = len(f.f_code.co_cellvars)
                if argval < n_cell:
                    arg = f.f_code.co_cellvars[argval]
                else:
                    arg = f.f_code.co_freevars[argval - n_cell]
            elif byteCode in dis.hasjrel:
                arg = f.f_lasti + argval
            elif byteCode in dis.hasjabs:
                arg = argval
            else:
                arg = argval
        return byteName, [arg] if arg is not None else [], opoffset
    ...
    def run_frame(self, frame):
        ...
        while True:
            ...
            if arguments:
                arguments = [*arguments, frame]
            else:
                arguments = [None, frame]
            print(f"[DEBUG] {byteName=}, {arguments=}")
